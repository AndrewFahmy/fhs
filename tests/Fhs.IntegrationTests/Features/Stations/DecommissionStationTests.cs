        ...
        Assert.Equal(
            [
                OutboxMessageSnapshot.For(
                    new StationDecommissioned(stationId, AppConstants.Data.AdminActorId, decommissionedAt)
                )
            ],
            [.. (await factory.OutboxForAsync(stationId, ct)).Select(OutboxMessageSnapshot.From)]
        );
    }
